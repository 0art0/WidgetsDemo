...
import type { DynamicComponent, Name, RpcPtr } from '@leanprover/infoview';
...
export function HtmlDisplay({html} : {html: Html}): JSX.Element {
...
interface TermDisplayWidgetRemoteProps {
    name: Name
    env: RpcPtr<'Lean.Environment'>
}

export default function TermDisplayWidgetRemote(props: TermDisplayWidgetRemoteProps) {
    const rs = useRpcSession()
    const st = useAsyncPersistent<Html>(async () => {
        const ret: Html =
            await rs.call('TermDisplayWidgetRemote.rpc', { name: props.name, env: props.env })
        return ret
    ...
    if (st.state === 'loading')
        ...
        return <HtmlDisplay html={st.value} />
}